...
T_green_2_start = T_green_1_start + init_dur + post_init_wait + pi_half*2 + post_MW_wait*3 + m*2 + gmSEQ.pi;
T_green_3_start = T_green_2_start + init_dur + pre_init_wait;
T_green_4_start = T_green_3_start + init_dur + post_init_wait + pi_half*2 + post_MW_wait*3 + n*2 + gmSEQ.pi;
...
T=[T_green_1_start+init_dur+post_init_wait,...
    post_MW_wait+m,...
    post_MW_wait+m,...
    post_MW_wait+pre_init_wait+init_dur*2+post_init_wait,...
    post_MW_wait+n,...
    post_MW_wait+n]; 
...
T=[T_green_1_start+init_dur+post_init_wait-q,...
    post_MW_wait+m-2*q,...
    post_MW_wait+m-2*q,...
    post_MW_wait+pre_init_wait+init_dur*2+post_init_wait-2*q,...
    post_MW_wait+n]; 
...
gmSEQ.CHN(5).NRise=1;
T=[T_green_4_start-post_MW_wait-pi_half-q]; 
DT=[pi_half+2*q];    
ConstructSeq(T,DT)
