...
post_seqMW_wait = 5000; %wait time between the last pulse and the readout
...
T=[pre_init_wait - pi - post_seqMW_wait, m_MW_T,n_MW_T]; 
DT=[pi,m_MW_DT,n_MW_DT];
T_start_for_minus_X_iq = sum(T)+sum(DT);
ConstructSeq(T,DT)


gmSEQ.CHN(4).PBN=PBDictionary('+X');
gmSEQ.CHN(4).NRise=(m+n)*4+3+1;
X_iq_rep1 = [4*tau+pi,6*tau+2*pi,4*tau+pi];
X_iq_rep2 = [2*tau, 4*tau+pi,6*tau+2*pi,4*tau+pi];
%first half of the differential measurement 
X_iq_m_start = MW_m_start; %this is the first iq start, note another 2*q will be substructed
if m == 0
    m_X_iq_T = [X_iq_m_start,2*tau]-2*q;
else
    m_X_iq_T = [X_iq_m_start,tau,X_iq_rep1, repmat(X_iq_rep2,1,m-1),tau]-2*q;
end
m_X_iq_DT = [pi_half,repmat(pi,1,m*4),pi_half]+2*q;
%second half of the differential measurement
...
T=[pre_init_wait - pi - post_seqMW_wait-q, m_X_iq_T,n_X_iq_T]; 
...
%first half of the differential measurement 
Y_iq_m_start = MW_m_start+pi_half+tau+pi+2*tau+q + (pre_init_wait - post_seqMW_wait); %this is the first iq start, note another 2*q will be substructed
if m == 0
...
%second half of the differential measurement 
Y_iq_n_start = MW_n_start+2*pi_half+2*pi+6*tau;
if n ==0 
...
gmSEQ.CHN(6).NRise=1;
T=[T_start_for_minus_X_iq-pi_half_diff-q]; 
DT=[pi_half_diff+2*q];    
ConstructSeq(T,DT)
